...
data = pd.concat([pd.read_csv(dataset_path + f'/dataindex_{i:0>2}.csv') for i in range(0, 37)]).drop('Unnamed: 0',axis=1)

# 중복이미지 제거 
data['path'] = data['path'].apply(lambda x: dataset_path + x.split('dataset')[1])

data['polygon1_x'] = data['polygon1'].apply(lambda x: re.findall(r"'x': (\d+.\d+)", x))
data['polygon1_y'] = data['polygon1'].apply(lambda y: re.findall(r"'y': (\d+.\d+)", y))

data['polygon2_x'] = data['polygon2'].apply(lambda x: re.findall(r"'x': (\d+.\d+)", x))
data['polygon2_y'] = data['polygon2'].apply(lambda y: re.findall(r"'y': (\d+.\d+)", y))

data['name'] = data['path'].apply(lambda x: x.split('/')[-1].split('-')[0])

if light: 
    # 중복이미지 제거 
    data = data.drop_duplicates('source',keep='first')
...
data['fold'] = 0
for fold, (tr_idx, val_idx) in enumerate(gkf.split(data, groups=data['name'])):
    data.loc[val_idx, 'fold'] = fold
    
from PIL import Image, ImageDraw
def polygon2mask(polygon_x, polygon_y):
    img = Image.new('L', (512, 512), 0)
    ImageDraw.Draw(img).polygon([(float(x), float(y)) for x, y in zip(list(polygon_x), list(polygon_y))], outline=1, fill=1)
    mask = np.array(img)
...
class TrainDataset(Dataset):
    ...
    def __getitem__(self, idx):
        images = cv2.imread(self.data.loc[idx]['path'])    
        images = cv2.cvtColor(images, cv2.COLOR_BGR2RGB).astype(np.float32)
        masks = polygon2mask(self.data.loc[idx]['polygon1_x'], self.data.loc[idx]['polygon1_y'])
    ...
    for epoch in range(CFG.epochs):
        ...
        if score > best_score:
            ...
            torch.save({'encoder': model_to_save.state_dict(), 
                        ...
                        OUTPUT_DIR+f'/{CFG.encoder_type}_{CFG.decoder_type}_fold{fold}_{CFG.light}.pth')
